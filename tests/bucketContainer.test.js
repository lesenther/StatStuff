...
const bucketContainer = require('../histogram/bucketContainer'); 

describe('Sorting functions', _ => {

    it('should sort string keys alphabetically', done => {
        const buckets = new bucketContainer([ 'z', 'b', 'x', 'c', 'y', 'a' ]);
        buckets.sortKeys();
        
        assert.equal(buckets.getKeys().join(''), 'abcxyz');
        done();
    });
    ...
    it('should sort integer keys numerically', done => {
        ...
        assert.equal(buckets.getKeys().join(''), '12345');
        done();
    });
    ...
    it('should sort floating point keys numerically', done => {
        ...
        assert.equal(buckets.getKeys().join(''), '1.22.23.24.14.25.2');
        done();
    });
    ...
    it('should sort mixed int and float keys numerically', done => {
        ...
        assert.equal(buckets.getKeys().join(''), '1.22.234.14.25.2');
        done();
    });
...
describe('Grouping functions', _ => {
    ...
    it('should create buckets', done => {
        ...
        assert.equal(buckets.getByKey(2).value, 3);
        done();
    });

    it('should put random integers into buckets', done => {
        const sampleData = skewnorm
        .rvSkewNorm(10000, 6, 5, 2, 0, 20)
        .map(val => Math.floor(val));

        const buckets = new bucketContainer(sampleData);
        
        buckets.forEach(bucket => {
            assert.equal(bucket.value, sampleData.filter(n => n.toString() === bucket.key.toString()).length);
        });
    ...
    it('should put string numbers, integers and floats of the same value into different groups', done => {
        const buckets = new bucketContainer([ 1, '1', 1.0, '1.000', 1.0000, true, '1', '1.00', '1.000' ]);

        assert.equal(buckets.getByKey(1).value, 3);
        assert.equal(buckets.getByKey('1').value, 2);
        assert.equal(buckets.getByKey('1.000').value, 2);
        done();
    });
...
describe('Mutation functions', _ => {
    ...
    it('should scale values', done => {
        ...
        buckets.scaleValues(2);
        assert.equal(buckets.getValues().join(''), '2468');
        buckets.scaleValues(.5);
        assert.equal(buckets.getValues().join(''), '1234');
        done();
    });
    ...
    it('should remove duplicate buckets', done => {
        ...
        buckets.mapKeys(key => Math.floor(key));
        buckets.rebuild();

        assert.equal(buckets.getKeys().length, 1);
        assert.equal(buckets.getByKey(1).key, 1);
        assert.equal(buckets.getByKey(1).value, 4);
        done();
    });

    it('should clear all the data', done => {
        const buckets = new bucketContainer([1, 1.1, 1.2, 1.3 ]);
        buckets.clear();

        assert.equal(buckets.getKeys().length, 0);
        done();
    });
    ...
    it('should delete a single bucket', done => {
        ...
        assert.equal(buckets.getKeys().length, 3);
        done();
    });
...
describe('Statistic functions', _ => {
    ...
    it('should get the correct max key length', done => {
        const buckets = new bucketContainer([ 1, 2, 2, 3, 3, 3, 'hello' ]);

        assert.equal(buckets.getMaxKeyLength(), 5);
        done();
    });
    ...
    it('should get the correct max bucket value', done => {
        ...
        assert.equal(buckets.getMaxValue(), 4);
        done();
    });
...
describe('Display functions', _ => {
    ...
    it('should print a horizontal histogram', done => {
        ...
        buckets.mapKeys(value => value.toFixed(2));
        buckets.sortKeys();
